...
class TransactionsPage {
  ...
  removeAccount() {
    ...
    if (!confirm('Вы уверены? Это действие удалит счет.')) {
      return;
    }
  ...
  removeTransaction( id ) {
    if (!confirm('Вы уверены? Это действие удалит транзакцию.')) {
      return;
    }
  ...
  formatDate(dateString) {
    ...
    return formatted.replace(/(\d{2}):(\d{2})$/, 'в $1:$2');
  }
  ...
  renderTransactions(data){
    const listTrans = document.querySelector('.content');
    listTrans.innerHTML = '';
    data.forEach(acc => {
      listTrans.insertAdjacentHTML('beforeend', this.getTransactionHTML(acc));
    })
  }
}